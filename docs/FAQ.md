...
Follow the migration notes in [FORMAT.md](FORMAT.md) and translate each `movie` block into `timeline.segments` with explicit `in`/`out` values. Most users can keep files under the `.emwy.yaml` extension and stay with a single timeline.
