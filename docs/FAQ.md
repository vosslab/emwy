...
Follow the migration notes in `docs/FORMAT.md` and match each `movie` block to playlists. Most users can translate segments into playlists with explicit `in`/`out` values.
