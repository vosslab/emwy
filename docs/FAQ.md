...
Follow the migration notes in [FORMAT.md](FORMAT.md) and match each `movie` block to playlists. Most users can translate segments into playlists with explicit `in`/`out` values and keep files under the `.emwy.yaml` extension.
