...
- Changed zoom toggle from shift-click to `z` key in seed collection and seed editor UIs. Shift-click was unreliable on macOS OpenCV. The `z` key now toggles 1.5x zoom centered on the frame; press `z` again to return to full view.
...
- Added severity-based target frame caps in `_mode_target()` in [emwy_tools/track_runner/cli.py](emwy_tools/track_runner/cli.py). Caps: high ~40 frames, medium ~80, low/none ~160. Uses `rank_target_frames_by_severity()` to pick the worst intervals with even spread.
