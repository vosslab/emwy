...
- **Zoom bounce fix**: bypass rate limiter when EMA smoothing is active in `direct_center_crop_trajectory()` in [emwy_tools/track_runner/tr_crop.py](emwy_tools/track_runner/tr_crop.py). When `crop_post_smooth_size_strength > 0` (EMA active), the `max_height_change` velocity limiter and three-mode zoom stabilization constraint are now skipped entirely. The EMA already bounds height change rate through smooth low-pass filtering; the rate limiter re-quantized the smooth signal into staircase steps, reintroducing bounce. Experiment 7d showed this directly: variant D (EMA + limiter combined) had the WORST pixel bounce rate (12.3/s) while variant B (EMA only) had the best (4.5/s). With the gate active, `crop_zoom_stabilization` and `crop_max_height_change` are inert when `alpha_size > 0`.
...
- Updated [tools/batch_encode_experiment.py](tools/batch_encode_experiment.py) to Experiment 8: validates the rate-limiter bypass. Two variants: B_baseline (EMA only, reproducing best 7d result) and D_gated (same config as 7d D with zoom_stab=True, but code gate bypasses limiter). Archived 7d D numbers used as historical failing reference.
- Created [emwy_tools/tests/test_tr_crop.py](emwy_tools/tests/test_tr_crop.py): 11 regression tests for the crop trajectory pipeline. Tests cover the EMA/rate-limiter gate (direct gate comparison, max_height_change independence proof, zoom_stabilization flag independence), scalar rate limiter at alpha_size=0, three-mode constraint at alpha_size=0, EMA convergence time, EMA constant signal preservation, and aspect ratio parsing.
...
- EMA smoothing and the rate limiter interact destructively when applied in series. The EMA produces a smooth curve; the rate limiter re-quantizes it into piecewise-linear staircases. The biased monotonicity freezes height for up to 5 frames before each small reversal, creating step-and-hold artifacts on the already-smooth signal. Mutual exclusion (bypass limiter when EMA active) is the correct fix.
- Experiment 8 confirms that the post-EMA rate limiter was responsible for the additional bounce seen in old D. With the limiter bypassed under EMA, D_gated becomes identical to the EMA-only baseline B on every metric (metadata and pixel) across all 7 videos. `zoom_stabilization` and `max_height_change` are fully inert when `alpha_size > 0`. Any remaining bounce is baseline EMA behavior and should be evaluated separately, not attributed to the limiter interaction.
- Composition regression noted in visual review: too much head room, feet cropped. This is a `crop_torso_anchor` tuning issue (currently 0.50 = centered), not related to the bounce fix. Deferred to a follow-up experiment.
...
- Experiment 8 pixel zoom assessment on IMG_3707: B_baseline and D_gated produce identical results (bounce_rate=6.125/s full, 5.158/s clip0, 6.277/s clip1; valid_frame_fraction=100%). Confirms the gate fully bypasses the limiter path.
