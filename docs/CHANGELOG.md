...
- Added `-I` short flag for `--seed-interval` in [emwy_tools/track_runner/cli.py](emwy_tools/track_runner/cli.py) on seed, target, and run subparsers. Example: `track_runner.py -i VIDEO seed -I 10`.
- Factored repeated subcommand args in [emwy_tools/track_runner/cli.py](emwy_tools/track_runner/cli.py) into helpers: `_add_seed_interval_arg()`, `_add_severity_arg()`, `_add_encode_args()`. Each shared arg group is now defined once and called where needed, reducing inline duplication across seed, edit, target, encode, and run subparsers.
