...
# Experiment 5 (virtual dolly cam): constraint-based stabilization.
#
# Prior experiments 1-4 used axis-isolated override passes (center_lock,
# fixed_crop, slow_size) which all produced visually indistinguishable
# output. The root cause: crop_fill_ratio=0.1 makes the crop 10x person
# height, so the subject is tiny and drifts freely within the frame.
#
# This experiment tests tighter fill ratio (the first-order fix) plus
# containment clamp and zoom constraint (second-order fixes) applied
# directly in direct_center_crop_trajectory().
#
# New config keys:
#   crop_fill_ratio: 0.3 or 0.4 (was 0.1)
#   crop_containment_radius: 0.20 (subject stays within 20% of crop center)
#   crop_max_height_change: 0.005 (max 0.5% height change per frame)
VARIANTS = {
	"A_old_baseline": {
		"description": "Current broken state: fill_ratio=0.1, no constraints",
		"overrides": {
			"crop_fill_ratio": 0.1,
	...
	"B_tight_030": {
		"description": "Tighter framing (0.3) + containment (0.20) + zoom constraint (0.005)",
		"overrides": {
			"crop_fill_ratio": 0.3,
	...
	"C_tight_040": {
		"description": "Tightest framing (0.4) + containment (0.20) + zoom constraint (0.005)",
		"overrides": {
			"crop_fill_ratio": 0.4,
			"crop_containment_radius": 0.20,
			"crop_max_height_change": 0.005,
	...
	"D_tight_030_no_contain": {
		"description": "Tighter framing only (0.3), no containment or zoom constraint",
		"overrides": {
			"crop_fill_ratio": 0.3,
			"crop_containment_radius": 0.0,
			"crop_max_height_change": 0.0,
